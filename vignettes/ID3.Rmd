...
"OK", I sigh, "fair enough. Wrong daughter." So I try my luck with Naomi, the seven year old: 
...
As mentioned above, when predicting, each node routes our mushroom according to an attribute? But how do we chose the attribute? That is where classification models differ. In ID3, we pick at each node the attribute with the highest *Information Gain*. In a nutshell, this is the attribute which splits the sample in possibly *purest* subset. For example, for mushrooms, "dots" might be a more sensible attribute then "organic". 

## Purity

We consider a subset to be completely pure if it contains only a single class. For example, if a subset contains only poisonous mushrooms, it is completely pure. In R, assuming that the last column contains the variable to be predicted, this can be written as:
