...
There are many more examples in the package's demos. Type `demo(package = "data.tree")` for a list of available demoes.
...
```{r}
secretaries <- c(3, 2, 8)
acme$Set(noSec = secretaries, 
         id = 1:3,
         filterFun = function(x) x$level == 2)
print(acme, "employees", "noSec", "id")
...
```{r}
traversal <- Traverse(acme, traversal = "post-order", filterFun = isNotLeaf)
...
The data.tree package has been built to support visualization, for rapid prototyping, and for other applications where development time saved is more important than computing time lost. Having said this, it becomes clear that big data and data.tree do not mary particularly well. Do not try to build a data.tree structure with a few million `Nodes`. Do not try to convert a JSON document with a size of a gigabyte to a data.tree structure.
...
2. `Clone` is similar to `Node` creation.
3. Traversing (`Traverse`, `Get`, `Set` and `Do`) is realtively cheap. 

The following times, just to give you an order of magnitude:
