...
print(acme)
```
Note that `Node` is an `R6` reference class. This has the following implications:
...
Just as with a `list`, we can add any custom attribute to any `Node` in a data.tree structure.
