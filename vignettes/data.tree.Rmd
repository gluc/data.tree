...
```{r}
data(acme)
acmeList <- acme$ToList()
```

You can call the `name` field and the `children` field differently, if you want:

```{r}
acmeList2 <- acme$ToList(nameName = 'id', childrenName = 'subNodes')
...
```{r}
acmeList3 <- acme$ToList(unname = TRUE)
acmeList3$children[[3]]$children[[1]]$cost
```

Finally, you can use `as.Node.List` to convert a nested list into a tree:
```{r}
as.Node(acmeList)
```
