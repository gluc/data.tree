...
Nothin stops you from setting a function as an attribute. This is a very powerful feature that lets you modify your tree without having to recalculate. Think of this as a hierarchical spreadsheet.
...
This, together with the `Set` method and recursion, becomes even more powerful:

```{r}
acme$Set(cost = c(function(self) sum(sapply(self$children, 
                                            function(x) GetAttribute(x, "cost")))), 
         filterFun = isNotLeaf)
print(acme, "cost")
...
Printing a tree is easy. `print` displays a tree in a tree-grid view. On the left, you have the hierarchy. Then you have a column per variable you want to print:
...
* You can set them on a `Node` using the `SetFormat` method. If you do this, then `print` will pick up the formatter as a default formatter
* You can add an explicit ad-hoc formater to the `Get` method (see below). This will overwrite default formatters set using the `SetFormat` method.
