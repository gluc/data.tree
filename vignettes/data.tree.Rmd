...
`data.tree` is a data structure, and as such it does not have a specialized plotting facility. However, R contains many packages that are well suited to plot hierarchies. One of them, DiagrammeR, is built into the data.tree package. 
You can access these features via the `plot` and `ToGraphViz` functions. For example:
...
Note that, by default, Node style attributes will be inherited.
However, inheritance can be avoided, as for the Accounting node in the following example:
