...
Once converted to a `data.tree` structure, we can use all its features to work with the hierarchical data. 
...
However, there is a number of other ways to get to a specific `Node`. We can access the children of a `Node` directly through the `Node$children` attribute: 
...
Furthermore, we can not only navigate by path, but also by other attributes. This is achieved with the `Climb` method:
...
Nothin stops you from setting a function as an attribute. This lets you modify your tree without having to recalculate. Think of this as a hierarchical spreadsheet, in which you can set formulas into cells.
...
Now, let's assume we discovered a new species:
...
This, together with the `Set` method and recursion, becomes a very powerful tool:
...
Printing a tree is easy, as you surely have seen in the previous sections. `print` displays a tree in a tree-grid view. On the left, you have the hierarchy. Then you have a column per variable you want to print:
...
* You can set them on a `Node` using the `SetFormat` method. If you do this, then the formatter will be picked up as a default formatter whenever you `print`, `Get`, convert to `data.frame`, etc. 
* You can add an explicit ad-hoc formater to the `Get` method (see below). This will overwrite default formatters previously set via the `SetFormat` method. You can also set the formatter to `identity` to void a formatter.
...
Formatting with the `Get` method:
...
`ExpectedCost` is expected to accept a `Node` as its first argument.


#### Using recursion

Recursion comes naturally with data.tree, and it's one of its core strengths: 

```{r}

ExpectedCost <- function(node) {
  result <- node$cost * node$p
  if(length(result) == 0) result <- sum(sapply(node$children, ExpectedCost))
  return (result)
}

print(acme, ec = acme$Get(ExpectedCost))

```

There is a built-in function that would make this example simpler: `Aggregate`. It is explained below.

#### Add parameters to the passed function

The `Get` method accepts an ellipsis (`...`). Any additional parameters with which `Get` is called will be handed over to the `ExpectedCost` function. This gives us more flexibility. For instance, we don't have to hard-code the `sum` function into `ExpectedCost`, but we can leave it to the caller to provide the function to use:

```{r}

AggregateCost <- function(node, fun = sum) {
  result <- node$cost * node$p
  if(length(result) == 0) result <- fun(sapply(node$children, AggregateCost))
  return (result)
}

print(acme, "cost", "p", ec = acme$Get(AggregateCost, fun = max))
...
Do is similar to `Get` in that it also traverses a tree in a specific traversal order. However, instead of fetching an attribute, it, surprise, does something. For example, we can tell the `Do` method to assign a value to each `Node` it traverses. This is especially useful if the attribute parameter is a function, as in the previous examples. For instance, we can store the calculated expected cost for later use and printing:

```{r}

acme$Do(function(node) node$expectedCost <- ExpectedCost(node))
...
Previously, we have used the `Get`, `Set` and `Do` methods in their OO-style version. This is often very convenient for quick access to variables. However, often times you want to re-use the same traversal for various operations. For this reason, we have the explicit traversal mode. It works like so:
...
You can prune sub-trees out of a tree. There are two variations of this:
* *temporary* pruning, e.g. just for printing: This is the `pruneFun` parameter, e.g. in `Get`
* *side effect* pruning, meaning that you modify your data.tree structure for good.
