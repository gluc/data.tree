...
Trees are ubiquitous in mathematics, computer science, data sciences, finance, and in many other fields. Trees are useful always when we are facing *hierarchical data*. Some examples where trees are useful:
...
R provides tree-like structures on various low levels. For example, environments can be seen as nodes in a tree. Also, R provides various packages that deal with tree-like structures (especially in the area of decision theory). Yet, there is no high-level hierarchical data structure that could be used equally conveniently and generically as, say, data.frame. 
...
This vignette uses an example revolving around [decision trees](http://en.wikipedia.org/wiki/Decision_tree).
