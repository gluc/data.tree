...
```{r}
it$Traverse("managerSalary", mode = "ancestor")
```
...
AvgMgrSalary <- function(node) {
  node$Traverse("managerSalary", mode = "ancestor") %>% mean
}
...
We can make this more flexible:
...
ExpectedCost <- function(node, fun = sum) {
  ...
  if(length(result) == 0) {
    if (node$isLeaf) result <- NA
      ...
      node$children %>% sapply(ExpectedCost) %>% fun -> result
    }
  }
  return (result)
}
...
And we can avoid repetitious calculations by piggy-backing on pre-calculated values. However, this requires us to traverse the tree in post-order mode:


```{r}

ExpectedCost <- function(node, variableName = "expectedCost", fun = sum) {
...
acme$Traverse(ExpectedCost, mode = "post-order", fun = mean, assign = "expectedCost")

acme$Traverse(NA, assign = "expectedCost")
