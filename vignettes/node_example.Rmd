...
Then, again using magrittr, the mean salary of the IT manager's business line is:

```{r}
it$Traject("managerSalary")

it$Traject("managerSalary") %>% mean

```

Or, more as an exercise than anything else, we could write our own level function:
```{r}
as.data.frame(acme) %>% 
  cbind(level = acme$Flatten("Traject", "name", length)) -> acmedf
