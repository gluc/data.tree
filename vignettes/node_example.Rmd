...
The same is called when printing a tree:
...
Above, we saw how we can add the name of an attribute to the ellipsis argument of the as.data.frame. However, we can also add the results of the Get method to the as.data.frame, but that giving us full flexibilitiy:

```{r}

as.data.frame(acme, 
              "level",
              cost = acme$Get("cost", format = PrintMoney),
...
The Get Method traverses the tree, can collects values from each node. It then returns a vector containing the collected values. 

Additional features of the Get Method are:
* execute a function on each node, and add the function's result to the returned vector
* execute a Method on each node, and add the Method's return value to the reutrned vector
