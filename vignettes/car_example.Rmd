...
```{r}
alternatives <-  list(accord_sedan = accord_sedan,
...
```{r}
goal$Find(c("Cost", "Fuel Costs"))$CalculatePreferences(fuelCostFunction)
sort(priorities(goal$Find(c("Cost", "Fuel Costs"))), decreasing = TRUE)
...
```{r}
pp <- goal$Find(c("Cost", "Maintenance Costs"))$GetChildCombinations()
#printForInput(pp)
...
preferenceMatrix <- AhpMatrix(pp)
goal$Find(c("Cost", "Maintenance Costs"))$SetChildPreferenceMatrix(preferenceMatrix)

priorities(goal$Find(c("Cost", "Maintenance Costs")))
...
goal$Find(c("Cost", "Resale Value"))$CalculatePreferences(resaleValuePreference)
priorities(goal$Find(c("Cost", "Resale Value")))
...
goal$Find(c("Safety", "Crash Rating"))$ApplyPreferenceMatrix(preferenceMatrix, 'crashRating')
priorities(goal$Find(c("Safety", "Crash Rating")))
```
...
goal$Find(c("Capacity", "Cargo Capacity"))$CalculatePreferences(cargoPreference)
priorities(goal$Find(c("Capacity", "Cargo Capacity")))
...
goal$Find(c("Capacity", "Passenger Capacity"))$ApplyPreferenceMatrix(AhpMatrix(pp), 'passenger')

priorities(goal$Find(c("Capacity", "Passenger Capacity")))
