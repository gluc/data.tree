...
jl$Do(
   function(node) {
     ...
     if (!node$isRoot && node$parent$type == 'chance') {
       SetEdgeStyle(node, label = paste0(node$name, " (", node$p, ")"))
     } else {
       SetEdgeStyle(node, label = node$name)
     }
      ...
      if (node$isRoot) {
        ...
        SetNodeStyle(node, 
                     style = "none",
                     shape = "none",
                     label = paste0( '$ ', format(node$payoff, scientific = FALSE, big.mark = ",")))

      }
   }
)

SetGraphStyle(jl, rankdir = "LR", fontname = "helvetica")
plot(jl)
...
Do(jl$leaves, function(node) edges(GetPhyloNr(node$parent, "node"), 
...
The data represents the [Flare](http://flare.prefuse.org/) class hierarchy, which is a code library for creating visualizations. The JSON is long, deeply nested, and complicated.
