...
Unfortunately, you need to take into account a few things. As a matter of fact, to pass the sub-trees from a child process to the main process, R needs to serialize the `Nodes` of the sub-tree, and this results in huge objects. As a result, collecting the sub-trees takes ages. 
...
```{r, echo= FALSE}
c(user = 7.406, system = 3.622, elapsed = 256.351)
```
