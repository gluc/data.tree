...
# World PopulationTreeMap (basic manipulation/visualisation)
...
We would like to limit the number of countries and sum the remaining population in a catch-all country called "Other".
...
## Adding Default Formatters to the data.tree structure 
...
Now, let's predict some classes. For this, we need a predict function, which will route data through our tree and make a prediction based on the leave it ends up:
...
# Jenny Lind (Decision Tree, YAML, plotting)
...
* how to plot my data.tree with the ape package
* how to plot my data.tree with the DiagrammeR package
...
# Bubble Chart (JSON, visualisation)
...
In this example, we print the files in the folder structure of the file system. As a special goodie, we'll show code that lets you build your own *R File Explorer*, an interactive tree / list widget that lets you expand folders.

## Print the files in a directory structure

First, let's read the files in a directory tree into R. The root path is the parent of the `vignettes` folder, i.e. the data.tree package folder itself:
...
## Display files with the interactive Listviewer html widget
...
The code generates 100 simulations of a 3 generation population. Individuals can inherit or develop a certain feature (e.g. colour blindness), with certain probabilities based on sex. We then plot the probability distribution of the feature in the last generation.
...
To speed up things a bit, we consider ratation, so that, say, the first move in a3 and a1 are considered equal, because they could be achieved with a 90 degree rotation of the board. This leaves us with only a3, b3, and b2 for the first move of player 1:
...
On average, how many different possibilities does a player have?
...
PrintBoard <- function(node) {
  ...
  field <- Get(t, "f")
  value <- Get(t, function(x) 10*x$player + (x$level - 1))
  mineV[field] <- value
