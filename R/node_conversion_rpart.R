...
#'
#' @return a \code{data.tree} object
#' @export
#'
#' @examples
#' if (require(rpart)) {
...
as.Node.rpart <- function(x, 
  ...
  leaf_labels <- x$functions$text(
    ...
    n      = frame$n[leaves], 
    use.n  = use.n)
  node_labels <- setNames(c(labels(x)[which(!leaves) + 1],
                            leaf_labels),
                          c(nodes[!leaves], nodes[leaves]))
  network_df  <- data.frame(from = node_labels[as.character(floor(nodes[-1L] / 2))],
                            to   = node_labels[as.character(nodes[-1L])])
  FromDataFrameNetwork(network_df)
}